...
import { handleRequest, Env } from './index.js';
...
export function startNodeServer() {
  ...
  const memoryKV = new Map<string, { val: string; exp?: number }>();
  const mockKV = {
  ...
  const segmentCache = new Map<string, { status: number; headers: [string, string][]; body: Buffer }>();
  const mockCache = {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      const response = await handleRequest(webReq, defaultEnv, ctx as any);

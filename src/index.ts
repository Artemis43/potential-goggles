...
export interface Env {
  API_BASE_URL: string;               // VPS API URL, e.g. https://api.slimestream.space
  JWT_SECRET?: string;                // Same secret the VPS signs stream tokens with (HS256)
  WORKER_SHARED_SECRET?: string;      // Authenticates this Worker to the VPS's /internal/* routes
  RATE_LIMIT_KV: KVNamespace;
  ALLOWED_ORIGINS?: string;           // Comma-separated
  MAX_SEGMENT_REQ_PER_MIN?: string;
  WORKER_ID?: string;                 // Optional worker identifier, e.g. "worker-1"
}
...
function requireSecret(env: Env, name: 'JWT_SECRET' | 'WORKER_SHARED_SECRET'): string {
  const value = env[name];
  if (!value) throw new Error(`${name} is not configured on this worker`);
  return value;
}
...
function memoryTokenCacheSet(key: string, value: { token: string; expiresAt: number }): void {
...
const jwtVerifyCache = new Map<string, { payload: StreamTokenPayload; cachedAt: number }>();
...
let _hmacVerifyKey: CryptoKey | null = null;
let _hmacVerifySecret: string = '';
let _hmacSignKey: CryptoKey | null = null;
let _hmacSignSecret: string = '';

async function getHmacVerifyKey(secret: string): Promise<CryptoKey> {
  if (_hmacVerifyKey && _hmacVerifySecret === secret) return _hmacVerifyKey;
  _hmacVerifyKey = await crypto.subtle.importKey(
    'raw',
...
async function getHmacSignKey(secret: string): Promise<CryptoKey> {
  if (_hmacSignKey && _hmacSignSecret === secret) return _hmacSignKey;
  _hmacSignKey = await crypto.subtle.importKey(
    'raw',
    new TextEncoder().encode(secret),
    { name: 'HMAC', hash: 'SHA-256' },
    false,
...
function checkRateLimitInMemory(key: string, maxPerMin: number): boolean {
  ...
  if (!entry || now - entry.windowStart >= windowMs) {
    if (rateLimitCounters.size >= RATE_LIMIT_MAP_MAX && !rateLimitCounters.has(key)) {
      ...
      if (oldestKey !== undefined) rateLimitCounters.delete(oldestKey);
    }
...
// ── Local JWT verification (HS256, WebCrypto) ───────────────────
...
const grantCache = new Map<string, number>(); // tokenHash -> validUntilMs
...
async function checkStreamGrant(token: string, env: Env): Promise<boolean> {
  const tokenHash = await sha256Hex(token);
  const now = Date.now();

  const memValidUntil = grantCache.get(tokenHash);
  if (memValidUntil && memValidUntil > now) return true;
...
async function verifyStreamToken(token: string, secret: string, env: Env): Promise<StreamTokenPayload | null> {
  ...
  if (cached && Date.now() - cached.cachedAt < JWT_CACHE_TTL) {
    ...
    try {
      ...
      const verified = JSON.parse(new TextDecoder().decode(base64UrlDecode(payloadB64!))) as StreamTokenPayload;
      if (verified.purpose !== 'stream' && verified.purpose !== 'admin_review') return null;
      ...
      if (jwtVerifyCache.size > 1000) {
        ...
        for (let i = 0; i < 200; i++) jwtVerifyCache.delete(keys[i]!);
      }
    } catch {
      return null;
    }
...
async function signChunkParams(videoId: string, chunkIndex: number, quality: string, fid: string, acc: string, secret: string): Promise<string> {
  ...
  if (sigCache.size > SIG_CACHE_MAX) {
    ...
    for (let i = 0; i < 500; i++) sigCache.delete(keys[i]!);
  }
...
async function getGDriveAccessToken(gdriveAccountId: string, env: Env): Promise<string | null> {
  const now = Date.now();
  ...
  const kvCached = await env.RATE_LIMIT_KV.get(cacheKey, 'json') as { accessToken: string; expiresAt: number } | null;
...
async function executeChunkRequest(
  request: Request,
  env: Env,
  ctx: ExecutionContext,
  cors: Record<string, string>,
  ...
  if (fileId.startsWith('http://') || fileId.startsWith('https://')) {
    ...
    if (accessToken) {
      ...
      const internalRes = await fetch(
        `${env.API_BASE_URL}/internal/storage-url/${encodeURIComponent(gdriveAccountId)}?path=${encodeURIComponent(fileId)}`,
        {
          headers: { 'X-Worker-Secret': workerSecret },
        },
      );
      ...
      if (internalRes.ok) {
        const { downloadUrl } = (await internalRes.json()) as { downloadUrl: string };
